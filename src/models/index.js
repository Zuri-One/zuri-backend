...
const models = {
  ...
  DoctorAvailability: require(path.join(
    __dirname,
  ...
  MedicationDispense: require(path.join(
    __dirname,
    'medication-dispense.model'
  )),
  PrescriptionMedications: require(path.join(
    __dirname,
    'prescription-medications.model'
  )),
...
const initializeModel = (Model) => {
  try {
    ...
    throw new Error(
      `Model ${Model.name} does not have a valid initialization pattern`
    );
  } catch (error) {
...
const initializedModels = modelInitializationOrder.reduce((acc, modelName) => {
  ...
  try {
    ...
    if (!acc[modelName]) {
      throw new Error(
        `Model ${modelName} initialization returned null or undefined`
      );
    }
...
Object.keys(initializedModels).forEach((modelName) => {
  if (typeof initializedModels[modelName].associate === 'function') {
    try {
      ...
      console.error(
        `Failed to create associations for model ${modelName}:`,
        error
      );
      throw error;
    }
...
Object.keys(initializedModels).forEach((modelName) => {
  if (modelName !== 'sequelize') {
    ...
    if (!model || !model.rawAttributes) {
      console.warn(
        `Warning: Model ${modelName} may not be properly initialized`
      );
    }
  }
});
